...
const Input = (props) => {
  ...
  } else if (props.class == "editInfo") {
    cls =
      "px-2.5  backdrop-blur-xl bg-white/40 border w-20 rounded-lg h-10 text-black";
  } else {
    cls =
      "px-2.5  backdrop-blur-xl bg-white/40 border w-full rounded-lg h-10 text-black";
  }
    ...
    if (props.label === "age") {
      ...
      if (value > 110) {
        ...
        if (!errors.current[0]) {
          setErrorMessage((prevValue) => [
            ...prevValue,
            "* only numbers can be used to describe age",
          ]);
  ...
  return (
    <div className=" flex flex-col items-center justify-center w-68 max-w-xs  w-5/6 mb-5 lowercase ">
      <input
        className="px-2.5  backdrop-blur-xl bg-white border w-full rounded-lg h-10 text-black"
        type={props.type}
        value={value}
