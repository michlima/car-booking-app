...
const Clock = (props)=> {
    ...
    return(
        <div className='flex items-center justify-center flex-col -translate-y-[1.5rem]'>
            ...
            {hours.map((e, index) => {
                    if(e == hour){
                        return <div key={index} className='duration-200  bg-red-400 px-3 my-1 snap-center'>{e}</div>
                    }
                    return(
                        <button key={index} onClick={() => changeHour(e)} className='duration-200  hover:bg-red-400 px-3 my-1 snap-center'>{e}</button>
                    )
                })}
            </div>
            ...
            {minutes.map((e, index) => {
                ...
                return(
                    <button key={index} onClick={() => changeMinute(e)} className='duration-200  hover:bg-red-400 px-3 my-1 snap-center'>{e}</button>
                )
            })}
        </div>
