...
const ReservationComponent = (props) => {
    ...
    if(schedule.month == data.month && schedule.day == data.day && schedule.year == data.year || props.canView){
        switch (schedule.car) {
            case 'Blue Van':
                cls = 'relative flex flex-col bg-blue-500 m-4 text-white rounded-lg w-10/12 shadow-lg'
                break;
            case 'Truck':
    ...
    return(
        <Link to='/focus-reservation' state={{ reservationData: props.object, userInfo: props.userInfo, onReturn:props.onReturn}} className={cls}>
            <div className='bg-gray-800 w-full flex flex-col text-1xl rounded-t-lg p-2 items-center text-white'>
                <a className='mr-2 text-1xl font-bold'>{schedule.reserverName}</a>
            ...
            {props.canView
            ?
                <p className='absolute bottom-2 right-2 '>{schedule.day} {schedule.month}</p>
            :
                <></>}
